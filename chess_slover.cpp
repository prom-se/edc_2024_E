        ...
        void serial_driver()
        {
            while (true)
            {
                try
                {
                    ...
                    if (robot_.task == 0x00)
                    {
                        vision_.dst_x = board_->get_pnp_position(board_->get_dst()).x;
                        vision_.dst_y = board_->get_pnp_position(board_->get_dst()).y;
                    }
                    else if (robot_.task == 0x01)
                    {
                        vision_.dst_x = board_->get_pnp_position(board_->get_dst_by_color(edc::BLACK) - 1).x;
                        vision_.dst_y = board_->get_pnp_position(board_->get_dst_by_color(edc::BLACK) - 1).y;
                    }
                    else if (robot_.task == 0x02)
                    {
                        vision_.dst_x = board_->get_pnp_position(board_->get_dst_by_color(edc::WHITE) - 1).x;
                        vision_.dst_y = board_->get_pnp_position(board_->get_dst_by_color(edc::WHITE) - 1).y;
                    }
                    else if (robot_.task == 0x03)
                    {
                        ...
                        src = board_->get_pnp_position(src_index);
                        dst = board_->get_pnp_position(dst_index);
                        src += fix;
